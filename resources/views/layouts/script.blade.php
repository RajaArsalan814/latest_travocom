...
<script>
    ...
    // Escalation Noti Count Start
    var count = "{{ $get_esc_count }}";
    var is_play = 0;
    var noti_counter = 0;
    ...
    setInterval(() => {
        ...
        $.ajax({
            url: "{{ url('/get_escalations_count/') }}" + '/' + count,
            type: 'GET',
            success: function(response) {
                // alert(response.count);
                count = response.count;
                if (count > 0) {
                    $('#noti_escalations').addClass("new");
                }
                // alert(count);

                $('#noti_count_escalations').empty().append(response.count);
                if (response.is_new == 1 && noti_counter == 0) {
                    toastr.info('New Escalation Received');
                    is_play = 1;
                } else {
                    is_play = 0;
                }
